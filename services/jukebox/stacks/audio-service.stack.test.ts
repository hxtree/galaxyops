...
import { AudioServiceStack } from './audio-service.stack';

describe('AudioServiceStack', () => {
  it('should match snapshot test', () => {
    const app = new cdk.App();
    const stack = new AudioServiceStack(app, 'MyTestStack');
    ...
    const customSnapshotSerializer = {
      ...
      print: (val: any) =>
        val.replace(/deployment\d{8}T\d{9}Z[^:]+/, 'deployment_UNIQUE_HASH'),
    };
