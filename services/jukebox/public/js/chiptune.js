...
ChiptunePlayer.prototype.xmp = {
  ...
  get_audio_source: function (player_ptr) {
    ...
    var get_buffer = function (player_ptr) {
      ...
      var buf_ptr = this.xmp.read(
        player_ptr,
        this.xmp.player_data[player_ptr].looping,
      );
  ...
  play: function (player_ptr, last_chunk_start, first_run, fixed_time) {
    ...
    if (first_run) {
      this.xmp.player_data[player_ptr].next_src =
        this.xmp.get_audio_source.bind(this)(player_ptr);
    }

    // Check for stop-flag or no more data
    // -> clean up
    if (
      this.xmp.player_data[player_ptr].stop === true ||
    ...
    if (!this.xmp.player_data[player_ptr].pause) {
      var last_chunk_duration = first_run
        ? 0
        : this.xmp.player_data[player_ptr].current_src.buffer.duration;
      var current_chunk_start = first_run
        ? context.currentTime
        : last_chunk_start + last_chunk_duration;
      if (fixed_time) {
        current_chunk_start = fixed_time;
      }
      this.xmp.player_data[player_ptr].last_src =
        this.xmp.player_data[player_ptr].current_src;
      this.xmp.player_data[player_ptr].current_src =
        this.xmp.player_data[player_ptr].next_src;

      this.xmp.player_data[player_ptr].current_src.start(current_chunk_start);
      this.xmp.player_data[player_ptr].current_src.timeScheduled =
        current_chunk_start;

      // schedule next chunk
      var current_chunk_duration =
        this.xmp.player_data[player_ptr].current_src.buffer.duration;
      var next_play = first_run
        ? (current_chunk_duration * 1000) / 2
        : current_chunk_duration * 1000;
      var timerId = setTimeout(
        this.xmp.play.bind(this, player_ptr, current_chunk_start),
        next_play,
      );
  ...
  pause: function (player_ptr) {
    ...
    if (this.xmp.player_data[player_ptr]) {
      this.xmp.player_data[player_ptr].last_src &&
        this.xmp.player_data[player_ptr].last_src.stop(0);
      this.xmp.player_data[player_ptr].current_src &&
        this.xmp.player_data[player_ptr].current_src.stop(0);
  ...
  resume: function (player_ptr) {
    ...
    if (
      ...
      this.xmp.player_data[player_ptr].pause &&
      !this.xmp.player_data[player_ptr].stop
    ) {
